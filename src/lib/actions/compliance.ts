...
import { setComplianceStatus } from "@/lib/repo/sustainability";

export type ComplianceState = { ok: boolean; message: string };

const schema = z.object({
  itemCode: z.string().trim().min(1).max(10),
  status: z.enum(["belum_mulai", "dalam_proses", "terbit", "akan_berakhir", "tidak_berlaku", "tidak_relevan"]),
...
export async function setComplianceStatusAction(
  _p: ComplianceState,
  fd: FormData,
  ...
  try {
    ...
    if (m === "FORBIDDEN") return { ok: false, message: "Hanya approver/super admin yang bisa mengubah status." };
